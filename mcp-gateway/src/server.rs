            ...
            loop {
                match client.initialize().await {
                    ...
                    Err(e) => {
                        retries += 1;
                        if retries >= 6 {
                            return Err(format!("Failed to initialize {} after {} retries: {}", name, retries, e));
                        }
...
async fn handle_tool_call(
    ...
    let instance = arguments.as_object_mut()
        .and_then(|obj| obj.remove("instance"))
        .and_then(|v| v.as_str().map(String::from));

    let guild_id = arguments.get("guild_id")
        .and_then(|v| v.as_str())
        .map(String::from);

    let target = state.router.resolve(
        instance.as_deref(),
        guild_id.as_deref(),
    ).await.map_err(|e| match e {
        RouteError::NoTarget => {
            let names: Vec<String> = state.router.instances.keys().cloned().collect();
            format!("No instance or guild_id specified. Available instances: {}", names.join(", "))
        }
...
async fn handle_list_instances(state: &GatewayState) -> Result<Value, String> {
    ...
    for (name, client) in backends.iter() {
        ...
        let guilds: Vec<&str> = guild_map.iter()
        ...
        lines.push(format!(
            "{} | {} | guilds: {}",
            name, status,
            if guilds.is_empty() { "none".to_string() } else { guilds.join(", ") }
        ));
    }
