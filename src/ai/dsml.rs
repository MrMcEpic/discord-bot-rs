...
static DSML_INVOKE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"<\x{ff5c}DSML\x{ff5c}invoke\s+name="([^"]+)"[^>]*>([\s\S]*?)<\x{ff5c}DSML\x{ff5c}/invoke>"#).unwrap()
});
...
static DSML_STRIP: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"<\x{ff5c}DSML\x{ff5c}[\s\S]*$").unwrap()
});
...
pub fn parse_dsml(content: &str) -> (Vec<DsmlToolCall>, String) {
    ...
    for cap in DSML_INVOKE.captures_iter(content) {
        ...
        calls.push(DsmlToolCall { name, arguments: args });
    }
