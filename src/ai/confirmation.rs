...
pub async fn request_confirmation(
	...
	if !has_perm {
		channel_id
			.say(&ctx.http, "You don't have permission to do that.")
			.await?;
		return Ok(false);
	}
	...
	let embed = CreateEmbed::new()
		...
		.footer(CreateEmbedFooter::new(format!(
			"Requested by {requester_name} · Expires in 30s"
		)));
	...
	loop {
		...
		if remaining.is_zero() {
			...
			let _ = confirm_msg
				.edit(
					&ctx.http,
					EditMessage::new().embed(expired_embed).components(vec![]),
				)
				.await;
			return Ok(false);
		}
		...
		match interaction {
			Some(interaction) => {
						...
						.create_response(
							...
							CreateInteractionResponse::Message(
								CreateInteractionResponseMessage::new()
									.content(
										"Only the person who requested this action can respond.",
									)
									.ephemeral(true),
							),
						)
				...
				let responder_name = interaction
					.member
					.as_ref()
					.and_then(|m| m.nick.as_deref())
				...
				let (color, title, footer_text) = if approved {
					(
						0x57f287,
						"Approved",
						format!("Approved by {responder_name}"),
					)
				} else {
					(
						0xed4245,
						"Cancelled",
						format!("Cancelled by {responder_name}"),
					)
				};
