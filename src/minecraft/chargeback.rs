...
async fn handle_chargeback(
    ...
    tracing::info!("Chargeback webhook received: player={}, tier={}, discord_id={:?}",
        payload.username, payload.tier, payload.discord_id);
    ...
    let restricted_role_id = match state.chargeback_config.restricted_role.parse::<u64>() {
        ...
        Err(_) => {
            ...
            return (StatusCode::INTERNAL_SERVER_ERROR, Json(WebhookResponse {
                success: false, error: Some("Server config error".to_string()),
            }));
        }
    };
    ...
    if let Some(ref discord_id_str) = payload.discord_id {
        if let Ok(uid) = discord_id_str.parse::<u64>() {
            ...
            match state.http.edit_member(state.guild_id, user_id, &edit, Some("Chargeback: roles stripped, user restricted")).await {
                Ok(_) => tracing::info!("Chargeback: stripped roles and restricted user {}", user_id),
                Err(e) => tracing::warn!("Chargeback: failed to modify roles for {}: {}", user_id, e),
            }
        }
    }
    ...
    (StatusCode::OK, Json(WebhookResponse { success: true, error: None }))
}
...
fn build_chargeback_buttons(payload: &ChargebackPayload) -> CreateActionRow {
    let ban_label = if payload.discord_id.is_some() { "🔨 Ban" } else { "🔨 Ban MC" };
...
pub async fn handle_button(
    ...
    let discord_field = embed.fields.iter()
        ...
        "ban" => {
            ...
            if let Some(user_id) = discord_user_id {
                ...
                match ctx.http.ban_user(guild_id, user_id, 0, Some(&reason)).await {
                    Ok(_) => tracing::info!("Chargeback: Discord-banned {} by {}", user_id, staff_user.name),
                    Err(e) => tracing::warn!("Chargeback: Discord ban failed for {}: {}", user_id, e),
                }
            }
                        ...
                        if let Some(ref mc_cfg) = data.minecraft_config {
                            if let Some(ref cb_cfg) = mc_cfg.chargeback_config {
                                if let Ok(ch) = cb_cfg.staff_channel.parse::<u64>() {
                                    let _ = ChannelId::new(ch).send_message(&ctx.http,
                                        CreateMessage::new().content(
                                            format!("⚠️ MC ban failed for UUID `{}`: {} {}", uuid, status, body)
                                        )
                                    ).await;
                                }
                            }
                        ...
                        if let Some(ref mc_cfg) = data.minecraft_config {
                            if let Some(ref cb_cfg) = mc_cfg.chargeback_config {
                                if let Ok(ch) = cb_cfg.staff_channel.parse::<u64>() {
                                    let _ = ChannelId::new(ch).send_message(&ctx.http,
                                        CreateMessage::new().content(
                                            format!("⚠️ MC ban failed for UUID `{}`: {}", uuid, e)
                                        )
                                    ).await;
                                }
                            }
