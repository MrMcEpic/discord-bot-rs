...
fn parse_id(s: &str) -> Result<u64, McpError> {
    s.parse::<u64>().map_err(|_| McpError::invalid_params(format!("Invalid ID: {s}"), None))
}
...
fn parse_duration_secs(s: &str) -> Result<i64, McpError> {
    ...
    let (num_str, mult) = if let Some(n) = s.strip_suffix('d') { (n, 86400i64) }
        else if let Some(n) = s.strip_suffix('h') { (n, 3600) }
        else if let Some(n) = s.strip_suffix('m') { (n, 60) }
        else if let Some(n) = s.strip_suffix('s') { (n, 1) }
        else { (s.as_str(), 60) };
...
impl DiscordTools {
    ...
    async fn send_message(&self, params: Parameters<SendMessageParams>) -> Result<CallToolResult, McpError> {
        ...
        Ok(CallToolResult::success(vec![Content::text(format!("Message sent (ID: {})", msg.id))]))
    }
    ...
    async fn delete_messages(&self, params: Parameters<DeleteMessagesParams>) -> Result<CallToolResult, McpError> {
        ...
        let messages = discord_call!(
            channel_id.messages(&*self.http, GetMessages::new().limit(count))
        );
        ...
        Ok(CallToolResult::success(vec![Content::text(format!("Deleted {} message(s)", ids.len()))]))
    }
    ...
    async fn delete_channel(&self, params: Parameters<ChannelIdParam>) -> Result<CallToolResult, McpError> {
        let channel_id = ChannelId::new(parse_id(&params.0.channel_id)?);
        discord_call!(self.http.delete_channel(channel_id, None));
        Ok(CallToolResult::success(vec![Content::text("Channel deleted")]))
    }
    ...
    async fn edit_channel(&self, params: Parameters<EditChannelParams>) -> Result<CallToolResult, McpError> {
        ...
        if let Some(topic) = p.topic { map.insert("topic".into(), serde_json::json!(topic)); }
        if let Some(nsfw) = p.nsfw { map.insert("nsfw".into(), serde_json::json!(nsfw)); }
        if let Some(sm) = p.slowmode_seconds { map.insert("rate_limit_per_user".into(), serde_json::json!(sm)); }
        if let Some(cat) = p.category_id { map.insert("parent_id".into(), serde_json::json!(cat)); }
        discord_call!(self.http.edit_channel(channel_id, &serde_json::Value::Object(map), None));
        Ok(CallToolResult::success(vec![Content::text("Channel updated")]))
    }
    ...
    async fn set_channel_permissions(&self, params: Parameters<SetChannelPermsParams>) -> Result<CallToolResult, McpError> {
        ...
        Ok(CallToolResult::success(vec![Content::text("Permissions set")]))
    }
    ...
    async fn edit_role(&self, params: Parameters<EditRoleParams>) -> Result<CallToolResult, McpError> {
        ...
        let mut map = serde_json::Map::new();
        if let Some(name) = p.name { map.insert("name".into(), serde_json::json!(name)); }
        if let Some(c) = p.color { map.insert("color".into(), serde_json::json!(c)); }
    ...
    async fn list_members(&self, params: Parameters<ListMembersParams>) -> Result<CallToolResult, McpError> {
        ...
        Ok(CallToolResult::success(vec![Content::text(format!("{} member(s):\n{}", lines.len(), lines.join("\n")))]))
    }
