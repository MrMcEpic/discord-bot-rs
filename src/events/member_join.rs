...
pub async fn handle_member_join(ctx: &Context, member: &Member, data: &Data) {
        ...
        match jr_config.role.parse::<u64>() {
            Ok(role_id) => {
                if let Err(e) = ctx.http.add_member_role(
                    ...
                    tracing::info!("Assigned join role to {} in guild {}", member.user.id, guild_id);
                }
            }
    ...
    // --- Welcome Message ---
    if let (Some(ref wc_config), Some(ref welcome_prompt)) = (&data.welcome_config, &data.welcome_prompt) {
        ...
        let api_key = data.config.deepseek_api_key.as_ref()
            ...
            .await
        {
            ...
            Ok(response) => {
                tracing::warn!("Welcome AI request failed with status {}", response.status());
            }
            Err(e) => {
