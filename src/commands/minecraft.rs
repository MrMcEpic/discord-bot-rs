...
fn require_mc_config(ctx: Context<'_>) -> Result<(String, String), BotError> {
	...
	let secret = ctx.data().config.mc_verify_secret.clone().ok_or_else(|| {
		BotError::Other("MC verification is not configured. Set `MC_VERIFY_SECRET` in .env".into())
	})?;
	Ok((url, secret))
}
...
pub async fn verify(
	...
	match result {
		...
		Err(e) => {
			ctx.say(format!("Could not reach the MC server: {e}"))
				.await?;
		}
	}
