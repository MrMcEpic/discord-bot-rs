...
use crate::music::embeds::{
	added_to_queue_embed, music_controls, now_playing_embed, queue_embed, status_footer,
};
...
async fn check_dj_mode(ctx: Context<'_>) -> Result<bool, BotError> {
	...
	ctx.say("DJ mode is enabled. You need the DJ role to use music commands.")
		.await?;
	Ok(true)
}
...
pub async fn play(
	...
	let (track, cookies_stale) = match resolve_track(&query, &display_name).await {
		...
		Err(e) => {
			...
			ctx.say("Couldn't find that song. Try a different search.")
				.await?;
			return Ok(());
		}
	};
	...
	if p.current.is_some() {
		if p.is_full() {
			ctx.say(format!("Queue is full (max {MAX_QUEUE_LENGTH} songs)."))
				.await?;
			return Ok(());
		}
		let pos = p.enqueue(track.clone());
		ctx.send(poise::CreateReply::default().embed(added_to_queue_embed(&track, pos)))
			.await?;
	} else {
		...
		let pctx = ctx
			.data()
			.playback_context(ctx.serenity_context(), guild_id, channel_id)
			.await;
...
pub async fn playlist(
	...
	let (tracks, cookies_stale) = match resolve_tracks(&query, &display_name, false).await {
		...
		Ok(_) => {
			ctx.say("No tracks found in that playlist.").await?;
			return Ok(());
		}
		Err(e) => {
	...
	if let Some(track) = first_track {
		...
		let pctx = ctx
			...
			.playback_context(ctx.serenity_context(), guild_id, channel_id)
			.await;
		if let Ok(handle) = voice::play_track(
			ctx.serenity_context(),
			guild_id,
			&track.url,
			&ctx.data().http_client,
		...
		let embed = CreateEmbed::new()
			...
			.field(
				"Now Playing",
				p.current.as_ref().map_or("Unknown", |t| &t.title),
				false,
			);
		let controls = music_controls(false, p.loop_mode);
		let reply = ctx
...
pub async fn skip(ctx: Context<'_>) -> Result<(), BotError> {
	...
	if let Some(title) = p.skip_current() {
		if let Some(next_track) = p.advance() {
			...
			let pctx = ctx
				.data()
				.playback_context(ctx.serenity_context(), guild_id, ctx.channel_id())
				.await;
			match voice::play_track(
				ctx.serenity_context(),
				guild_id,
			...
			ctx.say(format!("Skipped **{title}**. Queue is empty."))
				.await?;
		}
	} else {
...
pub async fn stop(ctx: Context<'_>) -> Result<(), BotError> {
	...
	ctx.say("Stopped playback, cleared queue, and left voice.")
		.await?;
	Ok(())
}
...
pub async fn shuffle(ctx: Context<'_>) -> Result<(), BotError> {
	...
	if len < 2 {
		...
		ctx.say(format!("Shuffled **{len}** songs in the queue."))
			.await?;
	}
